...
export class MockSttAdapter { constructor(text = "这是设备模拟器生成的测试转写。") { this.text = text; } async transcribe(_blob, { signal } = {}) { const started = Date.now(); return signal?.aborted ? result("cancelled", "mock", started, { message: "转写已取消" }) : result("success", "mock", started, { text: this.text }); } }
export class HttpSttAdapter {
  ...
  async transcribe(blob, { signal } = {}) {
    ...
    if (!(blob instanceof Blob)) return result("error", this.provider, started, { message: "录音数据无效" });
    if (blob.size > this.maxBytes) return result("error", this.provider, started, { message: `录音文件超过 ${Math.floor(this.maxBytes / 1024 / 1024)}MB 限制` });
    ...
    try { let response; for (let attempt = 0; attempt < 2; attempt += 1) { try { response = await this.fetchImpl(this.endpoint, { method: "POST", headers: { "Content-Type": blob.type || "application/octet-stream" }, body: blob, signal: controller.signal }); if (!response.ok) throw new Error(`HTTP ${response.status}`); break; } catch (error) { if (attempt || controller.signal.aborted) throw error; } } const data = await response.json(); if (typeof data.text !== "string") throw new Error("服务响应缺少 text 字段"); return result("success", this.provider, started, { text: data.text }); }
