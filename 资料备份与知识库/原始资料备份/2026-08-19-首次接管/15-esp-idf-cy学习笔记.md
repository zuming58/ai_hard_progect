...
> 整理时间：2026-08-18
> 整理人：Hermes（老王助手）
> 来源：GitHub 仓库 `CY-CHENYUE/esp-idf-cy`（已克隆到本目录 `/esp-idf-cy/`）
...
- **仓库克隆**：`F:/Hermes/AI硬件训练营/esp-idf-cy/`（含 `.git/`）
...
> 维护说明：本笔记基于 GitHub 仓库 `CY-CHENYUE/esp-idf-cy` @ commit `307361a`（2026-08-18 抓取）。CY 老师活跃迭代中，后续如有新版可在 `F:/Hermes/AI硬件训练营/esp-idf-cy/` 里 `git pull`，然后重新 cp 到 `~/.hermes/skills/esp-idf-cy/` 即可。